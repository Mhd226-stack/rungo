...
checkInternetConnection() {
  Connectivity().onConnectivityChanged.listen((List<ConnectivityResult> connectionState) {
...
registerUser() async {
  ...
  try {
    ...
    response.fields.addAll({
      ...
      "email": email,
      "device_token": fcm,
      "country": (countries.isNotEmpty) ? countries[phcode]['code'] : 'BF',
  ...
  try {
    var response =
        await http.post(Uri.parse('${url}api/v1/update/user/referral'),
            headers: {
              'Authorization':  bearerToken.isNotEmpty ? 'Bearer ${bearerToken[0].token}' : '',
...
Future geoCoding(double lat, double lng) async {
  dynamic result;
  try {
    ...
    if (response.statusCode == 200) {
      ...
      debugPrint('geoCoding status: ${val['status']}');
      debugPrint('geoCoding results: ${val['results']?.length}');
      if (val['results'] != null && val['results'].isNotEmpty) {
...
getlangid() async {
  ...
  try {
    var response = await http.post(
      Uri.parse('${base_url}api/v1/user/update-my-lang'),
      headers: {
        'Content-Type': 'application/json',
    ...
    if (response.statusCode == 200) {
      ...
      debugPrint('Erreur Validation: ${response.body}');
      var errorData = jsonDecode(response.body);
      if (errorData['errors'] != null) {
        var errors = errorData['errors'];
        result = errors[errors.keys.toList()[0]][0].toString();
      } else {
...
// polyline by AK
Future<Set<Polyline>> getPolylineList(
    LatLng startLocation, LatLng endLocation) async {
  List<LatLng> polylinePoints = [];
  try {
    String startLatitude = startLocation.latitude.toString();
    String startLongitude = startLocation.longitude.toString();
    String endLatitude = endLocation.latitude.toString();
    String endLongitude = endLocation.longitude.toString();

    var response = await http.get(Uri.parse(
        'https://maps.googleapis.com/maps/api/directions/json?origin=$startLatitude,$startLongitude&destination=$endLatitude,$endLongitude&key=$mapkey'));
    if (response.statusCode == 200) {
      var decodedResponse = jsonDecode(response.body);
      var points = decodedResponse['routes'][0]['overview_polyline']['points'];
      var decodedPoints = decodeEncodedPolyline(points);
  ...
  return Set<Polyline>.from(
      [Polyline(polylineId: PolylineId('route'), points: polylinePoints)]);
}
...
etaRequest() async {
  dynamic result;
  print((userRequestData.isNotEmpty)
      ...
      var distance = Geolocator.distanceBetween(
          ...
          (userRequestData.isNotEmpty)
              ...
              : addressList
                  ...
                  .latlng
                  .longitude);
      print('this is distance here after eta api ${distance}');
...
createRequest() async {
  // print('''${bearerToken[0].token.toString()}''');
  // print('''

  // .
  // ..
  // ''');
  // print('''${bearerToken[0].token.toString().substring(600)}''');
  ...
  // print('vehicle_type ${etaDetails[choosenVehicle]['zone_type_id']}');
  // print(
  //     'payment_opt ${(etaDetails[choosenVehicle]['payment_type'].toString().split(',').toList()[payingVia] == 'card') ? 0 : (etaDetails[choosenVehicle]['payment_type'].toString().split(',').toList()[payingVia] == 'cash') ? 1 : 2}');

  // print(
  //     'pick_address ${addressList.firstWhere((e) => e.type == 'pickup').address}');

  // print('stops ${jsonEncode(dropStopList)}');

  // print(
  //     'drop_address ${addressList.lastWhere((e) => e.type == 'drop').address}');

  // print('request_eta_amount ${etaDetails[choosenVehicle]['total']}');
...
getHistory(id) async {
  ...
  try {
    var response = await http.get(Uri.parse('${url}api/v1/request/history?$id'),
        headers: {'Authorization': bearerToken.isNotEmpty ? 'Bearer ${bearerToken[0].token}' : ''});
    if (response.statusCode == 200) {
...
updateProfile(name, email) async {
  ...
  try {
    ...
    response.headers
        .addAll({'Authorization': bearerToken.isNotEmpty ? 'Bearer ${bearerToken[0].token}' : ''});
    if (imageFile != null) {
      response.files
...
streamAdminchat() async {
  chatStream = FirebaseDatabase.instance
    ...
    if (value['from_id'].toString() != userDetails['id'].toString() &&
...
// make polyline from pickup and destination
...
class TriangleShape extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      height: height,
      width: width,
      child: Image.asset('assets/images/pickup_image.png'),
    );
  }
}
...
class _TrianglePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // To create a rounded cone effect at the bottom
    final Rect ovalRect = Rect.fromLTWH(0, 0, size.width, 0);
    path.arcTo(ovalRect, degreeToRadians(0), degreeToRadians(180), false);
...
class InvoicePriceListItem extends StatelessWidget {
  const InvoicePriceListItem({super.key, required this.child});
  final Widget child;
...
class IosBackButton extends StatelessWidget {
  const IosBackButton({super.key, this.width, this.height});
  final double? width;
  final double? height;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      height: height ?? Responsive.width(8, context),
      decoration:
...
class ButtonBottomSpace extends StatelessWidget {
  const ButtonBottomSpace({super.key, this.height});
  final double? height;
